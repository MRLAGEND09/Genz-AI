...
const Sidebar = ({ expand, setExpand, show = true }) => {
  ...
  const groupChatsByDate = (chats) => {
    ...
    chats.forEach(chat => {
      ...
      } else if (isAfter(chatDate, subDays(new Date(), 7))) {
  ...
  return (
    <div className={`flex flex-col justify-between bg-[#212327] pt-7 transition-all z-50 max-md:absolute max:h-screen
      ...
      <div>
        <div className={`flex ${expand ? "flex-row gap-10" : "flex-col items-center gap-8"}`}>
          ...
          <div onClick={() => setExpand(!expand)}
            className='group relative flex items-center justify-center hover:bg-gray-500/20 transition-all duration-300 h-9
            w-9 aspect-square rounded-lg cursor-pointer'>
        ...
        <div className={`mt-8 text-white/25 text-sm ${expand ? "block" : "hidden"}`}>
          ...
          <div>
            {chats && chats.length > 0 ? (
              Object.entries(groupChatsByDate(chats)).map(([group, groupChats]) => (
                groupChats.length > 0 && (
                  <div key={group} className="mb-4">
                    <p className="text-white/40 text-xs font-semibold px-2 mb-1">{group}</p>
                    {groupChats.map(chat => (
                      <ChatLabel
                        key={chat._id}
                        id={chat._id}
              ...
              <p className="text-xs text-white/40 px-2">No chats yet</p>
            )}
          </div>
        </div>
      </div>
